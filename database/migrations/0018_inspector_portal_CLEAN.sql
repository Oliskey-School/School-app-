...
-- Migration 0018: Inspector & Inspection System (CLEAN VERSION)

-- =====================================================
-- CLEAN SLATE: Drop everything in correct order
-- =====================================================

-- 1. Drop policies first (they prevent table drops)
DROP POLICY IF EXISTS "Public can view responses" ON public.inspection_responses;
DROP POLICY IF EXISTS "Inspectors can manage responses" ON public.inspection_responses;
DROP POLICY IF EXISTS "Schools can view their inspection responses" ON public.inspection_responses;
DROP POLICY IF EXISTS "Public can view active templates" ON public.inspection_checklist_templates;
DROP POLICY IF EXISTS "Public can view inspections" ON public.inspections;
DROP POLICY IF EXISTS "Schools can view their inspections" ON public.inspections;
DROP POLICY IF EXISTS "Inspectors can manage own inspections" ON public.inspections;
...
CREATE POLICY "Inspectors can manage responses" ON public.inspection_responses
    FOR ALL USING (
        inspection_id IN (
            ...
            WHERE inspector_id IN (SELECT id FROM public.inspectors WHERE user_id = auth.uid())
        )
    );
...
('Teaching Quality & Curriculum', 'Teaching Quality', '[
...
('Health, Safety & Child Protection', 'Health & Safety', '[
...
BEGIN
    ...
    SELECT 
        ...
        COALESCE(SUM((jsonb_array_elements(ict.questions)->>'max_points')::INTEGER), 0)::INTEGER as max_score,
        CASE 
            WHEN COALESCE(SUM((jsonb_array_elements(ict.questions)->>'max_points')::INTEGER), 0) > 0
            THEN (COALESCE(SUM(score), 0)::DECIMAL / COALESCE(SUM((jsonb_array_elements(ict.questions)->>'max_points')::INTEGER), 1)::DECIMAL * 100)
            ELSE 0
        END as percentage
...
-- SUCCESS!
-- =====================================================
SELECT '🎉 Inspector Portal Created!' as status,
       '📊 4 categories (100 pts)' as categories,
