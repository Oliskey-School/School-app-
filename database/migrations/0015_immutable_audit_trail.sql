...
-- Drop existing objects to allow re-running this migration
-- Drop functions first (they don't depend on tables)
DROP FUNCTION IF EXISTS public.prevent_log_mutation() CASCADE;
DROP FUNCTION IF EXISTS public.process_audit_log() CASCADE;

-- Drop table (this will cascade drop all triggers on it)
DROP TABLE IF EXISTS public.audit_logs CASCADE;

-- Drop triggers on other tables only if those tables exist
DO $$
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'academic_records') THEN
        DROP TRIGGER IF EXISTS tr_audit_records ON public.academic_records;
    END IF;
END $$;
