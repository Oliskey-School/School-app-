-- DATA SEEDING SCRIPT for 'Demo Academy' (FIXED SCHEMA)
...
DECLARE
    ...
    v_class_map_id uuid;
BEGIN
    -- 1. Create Subjects (Removed 'description')
    ...
    -- 3. Create Teachers (Removed 'qualification', fixed 'subject_specialization' to ARRAY)
    ...
    -- 6. Create Fees (Removed 'total_fee', 'term')
    ...
    -- 7. Create Attendance (Used 'class_id' instead of 'class_name')
    -- We need to map students to classes based on grade match? or just pick a class?
    -- For simplicity, let's map 'Grade X' class to students of grade X
    FOR v_student_rec IN (SELECT id, grade FROM students WHERE school_id = v_school_id) LOOP
        -- Find class ID for this student's grade
        SELECT id INTO v_class_map_id FROM classes 
        WHERE school_id = v_school_id AND grade_level = v_student_rec.grade 
        LIMIT 1;
