-- DATA SEEDING SCRIPT for 'Demo Academy' (FIXED SCHEMA v7: With Auth Users)
...
DECLARE
    ...
    v_class_map_id uuid;
BEGIN
    -- 0. Helper to create auth user if likely missing (we generate random UUIDs so they are new)
    -- We can't easily check email in auth.users from PL/pgSQL without permissions sometimes, 
    -- but assuming we generate new UUIDs, we are safe.
