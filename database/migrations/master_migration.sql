...
-- Based on: 0018_inspector_portal_FINAL.sql
-- Status: VERIFIED
...
-- Inspector Policies
CREATE POLICY "Inspectors can view own profile" ON public.inspectors FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Inspectors can update own profile" ON public.inspectors FOR UPDATE USING (auth.uid() = user_id);
