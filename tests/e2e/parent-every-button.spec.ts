...
// ParentDashboard only exposes its audit hooks when audit mode is enabled.
...
// Recovery when a full-screen view unmounts the ParentDashboard. The dashboard
// now deletes PARENT_NAVIGATE on unmount, so an absent hook is a reliable signal
// to reload (which restores the demo session) rather than re-login (which would
// trip the demo-login rate limiter).
...
test('every parent screen — every clickable button passes (no 5xx, no pageerror, no ErrorBoundary)', async ({ page, baseURL }) => {
    ...
    fs.writeFileSync(path.join(outDir, 'parent-every-button.json'),
    ...
    const md: string[] = [];
    md.push(`# Parent — Every Clickable Button E2E`);
    md.push('');
    ...
    fs.writeFileSync(path.join(outDir, 'parent-every-button.md'), md.join('\n'));

    console.log(`\n>>> ${totalPass}/${totalClicked} buttons PASS, ${totalFailed} FAIL, ${totalSkipped} SKIP across ${viewResults.length} views`);
    console.log(`>>> Detail: .playwright-results/parent-every-button.md`);
