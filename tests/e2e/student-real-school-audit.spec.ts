...
/**
 * Production-readiness audit of the STUDENT dashboard against a brand-new,
 * REAL (non-demo) tenant created through the actual onboarding wizard.
 * Adapted from student-every-button.spec.ts — swaps demo login for real login.
 */
...
const STUDENT_EMAIL = 'kelechi.eze.z2vuq1@student.school.com';
const STUDENT_PASSWORD = '8b2a1bf05a8d2703';

async function loginAsRealStudent(page: Page, baseURL: string) {
    await page.goto(baseURL, { waitUntil: 'domcontentloaded' });
    await page.waitForSelector('text=/Sign In|School Portal/i', { timeout: 15_000 });
    await page.locator('input[name="email_prevent_autofill"]').fill(STUDENT_EMAIL);
    await page.locator('input[name="password_prevent_autofill"]').fill(STUDENT_PASSWORD);
...
async function ensureStudentDashboard(page: Page, baseURL: string) {
    ...
    await loginAsRealStudent(page, baseURL);
}
...
test('REAL school — every student screen — every clickable button passes', async ({ page, baseURL }) => {
    ...
    console.log(`>>> Discovered ${views.length} student views: ${views.join(', ')}`);
    ...
    fs.writeFileSync(path.join(outDir, 'student-real-school-audit.json'),
    ...
    const md: string[] = [];
    md.push(`# Student — REAL school audit — Every Clickable Button E2E`);
    md.push('');
    ...
    fs.writeFileSync(path.join(outDir, 'student-real-school-audit.md'), md.join('\n'));

    console.log(`\n>>> ${totalPass}/${totalClicked} buttons PASS, ${totalFailed} FAIL, ${totalSkipped} SKIP across ${viewResults.length} views`);
    console.log(`>>> Detail: .playwright-results/student-real-school-audit.md`);
