...
/**
 * Mobile-responsive sweep: every admin view, at every phone/small-tablet
 ...
 * confirmed visually afterward, without screenshotting all 189 views blind.
 */

const VIEWPORTS = [
    { name: '375x812', width: 375, height: 812 },   // iPhone SE / small phone
    { name: '414x896', width: 414, height: 896 },   // large phone
    { name: '768x1024', width: 768, height: 1024 }, // small tablet portrait
];
...
test('every admin view — mobile breakpoints have no horizontal overflow / overlap / icon sizing bugs', async ({ page, baseURL }) => {
    ...
    const shotDir = path.join(outDir, 'admin-mobile-shots');
    ...
    for (let i = 0; i < views.length; i++) {
        ...
        if (!helperPresent) {
            try { await loginAsDemoAdmin(page, baseURL!); }
            catch { continue; }
        }
        ...
        for (const vp of VIEWPORTS) {
            ...
            const result = await page.evaluate(() => {
                ...
                if (overflowPx > 4) {
                    // Find up to 5 leaf-ish elements whose right edge exceeds the viewport
                ...
                for (const el of icons) {
                    const r = el.getBoundingClientRect();
                    if (r.width === 0 && r.height === 0) continue; // hidden/display:none, not a bug
    ...
    fs.writeFileSync(path.join(outDir, 'admin-mobile-scan.json'), JSON.stringify(findings, null, 2));

    const md: string[] = [];
    md.push(`# Admin — Mobile Responsive Scan`);
    md.push('');
    ...
    fs.writeFileSync(path.join(outDir, 'admin-mobile-scan.md'), md.join('\n'));

    console.log(`\n>>> ${findings.length} flagged (view, viewport) pairs out of ${views.length * VIEWPORTS.length} checks`);
    console.log(`>>> Detail: .playwright-results/admin-mobile-scan.md`);
});
