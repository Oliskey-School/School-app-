...
/**
 * Mobile-responsive sweep: every teacher view, at every phone/small-tablet
...
test('every teacher view — mobile breakpoints have no horizontal overflow / overlap / icon sizing bugs', async ({ page, baseURL }) => {
    ...
    const shotDir = path.join(outDir, 'teacher-mobile-shots');
    ...
    for (let i = 0; i < views.length; i++) {
        ...
        if (!helperPresent) {
            try { await loginAsDemoTeacher(page, baseURL!); }
            catch { continue; }
        }
    ...
    fs.writeFileSync(path.join(outDir, 'teacher-mobile-scan.json'), JSON.stringify(findings, null, 2));

    const md: string[] = [];
    md.push(`# Teacher — Mobile Responsive Scan`);
    md.push('');
    ...
    fs.writeFileSync(path.join(outDir, 'teacher-mobile-scan.md'), md.join('\n'));

    console.log(`\n>>> ${findings.length} flagged (view, viewport) pairs out of ${views.length * VIEWPORTS.length} checks`);
    console.log(`>>> Detail: .playwright-results/teacher-mobile-scan.md`);
});
