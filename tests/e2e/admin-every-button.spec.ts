...
test('every admin screen — every clickable button passes (no 5xx, no pageerror, no ErrorBoundary)', async ({ page, baseURL }) => {
    ...
    for (let i = 0; i < views.length; i++) {
        ...
        // Ensure helper still mounted (some views unmount AdminDashboard)
    ...
    fs.writeFileSync(path.join(outDir, 'admin-every-button.json'),
    ...
    const md: string[] = [];
    md.push(`# Admin — Every Clickable Button E2E`);
    md.push('');
    ...
    fs.writeFileSync(path.join(outDir, 'admin-every-button.md'), md.join('\n'));

    console.log(`\n>>> ${totalPass}/${totalClicked} buttons PASS, ${totalFailed} FAIL, ${totalSkipped} SKIP across ${viewResults.length} views`);
    console.log(`>>> Detail: .playwright-results/admin-every-button.md`);
