...
async function loginAsRealAdmin(page: Page, baseURL: string) {
    await page.goto(baseURL, { waitUntil: 'domcontentloaded' });
    await page.locator('input[name="email_prevent_autofill"]').fill('owner.z2vuq1@e2e-live.test');
    await page.locator('input[name="password_prevent_autofill"]').fill('RealSchool!789');
...
async function ensureAdminDashboard(page: Page, baseURL: string) {
    ...
    await loginAsRealAdmin(page, baseURL);
}
...
test('REAL SCHOOL (Bright Future College Z2VUQ1) — every admin screen — every clickable button passes', async ({ page, baseURL }) => {
    test.setTimeout(150 * 60 * 1000); // up to 150 min — admin has 189 views
    ...
    console.log(`>>> Discovered ${views.length} admin views`);
    ...
    for (let i = 0; i < views.length; i++) {
        ...
        if (!helperPresent) {
            try { await ensureAdminDashboard(page, baseURL!); }
            catch (e: any) {
        ...
        const nav = await page.evaluate(async (v) => {
            try { (window as any).ADMIN_NAVIGATE(v, v, {}); return { ok: true }; }
        ...
        for (let bi = 0; bi < buttonsFound; bi++) {
            ...
            const onView = await page.evaluate((v) => {
                const cur = (window as any).__CURRENT_ADMIN_VIEW__;
                return cur ? cur === v : true;
            }, view).catch(() => true);
            if (!onView) {
                await page.evaluate((v) => (window as any).ADMIN_NAVIGATE?.(v, v, {}), view).catch(() => { });
    ...
    fs.writeFileSync(path.join(outDir, 'admin-real-school-audit.json'),
    ...
    const md: string[] = [];
    md.push(`# Admin (REAL SCHOOL) — Every Clickable Button E2E`);
    md.push('');
    ...
    fs.writeFileSync(path.join(outDir, 'admin-real-school-audit.md'), md.join('\n'));

    console.log(`\n>>> ${totalPass}/${totalClicked} buttons PASS, ${totalFailed} FAIL, ${totalSkipped} SKIP across ${viewResults.length} views`);
    console.log(`>>> Detail: .playwright-results/admin-real-school-audit.md`);
