...
/**
 * Mobile-responsive sweep: every student view, at every phone/small-tablet
...
test('every student view — mobile breakpoints have no horizontal overflow / overlap / icon sizing bugs', async ({ page, baseURL }) => {
    ...
    const shotDir = path.join(outDir, 'student-mobile-shots');
    ...
    for (let i = 0; i < views.length; i++) {
        ...
        if (!helperPresent) {
            try { await loginAsDemoStudent(page, baseURL!); }
            catch { continue; }
        }
    ...
    fs.writeFileSync(path.join(outDir, 'student-mobile-scan.json'), JSON.stringify(findings, null, 2));

    const md: string[] = [];
    md.push(`# Student — Mobile Responsive Scan`);
    md.push('');
    ...
    fs.writeFileSync(path.join(outDir, 'student-mobile-scan.md'), md.join('\n'));

    console.log(`\n>>> ${findings.length} flagged (view, viewport) pairs out of ${views.length * VIEWPORTS.length} checks`);
    console.log(`>>> Detail: .playwright-results/student-mobile-scan.md`);
});
