...
const AuthenticatedApp: React.FC = () => {
  ...
  return <ErrorBoundary><Suspense fallback={<LoadingScreen />}>
    ...
    <VerificationGuard>
      ...
      {subscriptionGate.isLocked && !window.location.pathname.startsWith('/subscription') ? <SubscriptionLockScreen /> : <>
        {renderDashboard}
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const runBackgroundInitialization = () => {
      runMigrations().then(() => cacheCleanupScheduler.start()).catch((error) => console.error('❌ Initialization failed:', error));
