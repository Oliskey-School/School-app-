...
- **Offline Mode:** The PWA successfully caches operations via DexieDB and syncs to the database on reconnection.
...
4. **User Invites:** The `POST /api/invite-user` endpoint operates efficiently using the the backend service-role key to securely onboard new staff and parents.
...
The Express backend perfectly mirrors the expected REST contracts defined by the frontend `HybridApiClient`. There are **zero missing APIs**. The system successfully bridges Prisma-enforced scoping with Express business logic.
