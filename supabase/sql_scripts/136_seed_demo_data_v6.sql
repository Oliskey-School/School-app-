-- DATA SEEDING SCRIPT for 'Demo Academy' (FIXED SCHEMA v6)
...
BEGIN
    ...
    -- 2. Create Classes (Added 'level', Changed 'grade_level' to 'grade')
    ...
    -- 3. Create Teachers (Profiles: full_name, role Capitalized. No email/user_type)
    ...
    FOR i IN 1..20 LOOP
        ...
        -- Insert Profile (Capitalized Role)
        INSERT INTO profiles (id, school_id, full_name, role)
        VALUES (
            v_student1_id, 
            v_school_id, 
    ...
    -- Map student grade to class grade
    FOR v_student_rec IN (SELECT id, grade FROM students WHERE school_id = v_school_id) LOOP
        -- Find class ID for this student's grade (Changed 'grade_level' to 'grade')
