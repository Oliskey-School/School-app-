-- Migration: Fix get_dashboard_stats to perfectly match List Page arrays
-- Description: Removes the strict 'Active' requirement from the count total so the dashboard precisely matches the Manage Students and Manage Teachers lists.
...
DECLARE
    ...
    v_class_trend INT;
BEGIN
    -- 1. Students Count (Match StudentListScreen - All statuses)
    SELECT COUNT(*) INTO v_total_students
    FROM public.students
    WHERE school_id = p_school_id
    AND (p_branch_id IS NULL OR branch_id = p_branch_id);

    -- 2. Teachers Count (Match TeacherListScreen - All statuses)
