-- Migration: Enhance School Signup Trigger for Branches
-- Description: Updates handle_new_school_signup to support multiple branches during registration.
...
BEGIN
    ...
    BEGIN
        ...
        v_branch_names := NEW.raw_user_meta_data->'branch_names';
        ...
        -- 2. Create Branches
        IF v_branch_names IS NOT NULL AND jsonb_array_length(v_branch_names) > 0 THEN
            -- Insert branches from provided names
            INSERT INTO public.branches (school_id, name, is_main, location)
            SELECT v_school_id, b_name, (position = 1), COALESCE(v_address, 'Branch Address')
            FROM jsonb_array_elements_text(v_branch_names) WITH ORDINALITY AS b(b_name, position);
        ELSE
            -- Default Main Branch
            INSERT INTO public.branches (school_id, name, is_main, location)
        ...
        -- 5. Template Cloning (Optional, depends on system config)
        SELECT id INTO v_template_school_id FROM public.schools 
        WHERE id = 'd0ff3e95-9b4c-4c12-989c-e5640d3cacd1' OR slug LIKE '%demo%' 
        LIMIT 1;

        IF v_template_school_id IS NOT NULL AND v_template_school_id != v_school_id THEN
