...
- Don't blind-patch ID-generation logic that a project's own governing doc calls out as a non-negotiable core invariant (e.g. a documented Global ID format). Document the race precisely (file, lines, exact failure mode) as a recommendation instead of rushing a fix without dedicated design/testing time — then, once the exact failure mode is understood (see below), it's usually a contained fix, not a redesign.
- **Fix pattern that closes it without a schema change:** a Postgres transaction-scoped advisory lock (`SELECT pg_advisory_xact_lock(hashtext($1)::bigint)`, keyed per partition — e.g. `school+branch+role`) serializes concurrent generation for the same partition and auto-releases at commit/rollback. Critical constraint: **the lock is only effective if the ID generation and the row insert that consumes it happen inside the same `prisma.$transaction`** — acquiring the lock, returning the ID, and letting the caller commit before inserting elsewhere protects nothing. When auditing, grep every call site of the generator function, not just the obvious ones — this codebase had 8 call sites across 6 files (`auth.service.ts` signup + admin-create, `teacher.service.ts` create + self-heal, `parent.service.ts`, `student.service.ts`, `onboarding.service.ts`, `branchTransfer.service.ts`, `invite.routes.ts`), and several silently generated the ID *outside* any transaction, generated it in one transaction and inserted in a separate later query, or generated it with `tx` never actually passed through despite being inside a `$transaction` block already. Every one of those needed the generation call moved inside (or its existing `tx` param finally wired through).
- A read-only "preview next ID" endpoint (shown in a UI form before the real submit) does not need this treatment — nothing is inserted there, so there's no race to close.
