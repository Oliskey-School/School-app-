...
// there is NO `authenticator` export, so `_otplib.authenticator ?? …` always fell
// through to a stub whose keyuri() returned ''. Worse, the CJS `require('otplib')`
// path throws "s is not defined" under Node 20, which is what
// GET /auth/2fa/setup was actually returning as a 400. Load via dynamic ESM
// import and adapt the real API behind the same small interface.
...
function getAuthenticator(): Promise<any> {
    if (!_authPromise) {
        _authPromise = import('otplib').then((otp: any) => ({
            ...
            // verifySync returns { valid, delta, … } — NOT a boolean. Using the
            // object directly would make EVERY code truthy, i.e. accepted.
...
export class AuthService {
    ...
    static async verifyAndEnable2FA(userId: string, code: string) {
        // MUST use the raw client: the global Prisma extension strips
        // two_factor_secret from every result, so the normal client reads it as
        // undefined and this always threw "2FA not initiated" — even for a
        // correct code. Same reason login uses getRawPrisma() for password_hash.
    ...
    static async verify2FALogin(mfaToken: string, code: string) {
        try {
            ...
            // throw for EVERY user — anyone who enabled it would be locked out.
