...
const ParentProfileScreen: React.FC<ParentProfileScreenProps> = ({ onLogout, navigateTo, forceUpdate }) => {
  ...
  const renderActiveSetting = () => {
    switch (activeSetting) {
        case 'editParentProfile': return <EditParentProfileScreen />;
      ...
      <div className={`w-full md:w-[400px] md:flex-shrink-0 bg-gray-50 flex flex-col ${activeSetting ? 'hidden md:flex' : 'flex'}`}>
        <div className="flex-grow p-4 space-y-4 overflow-y-auto">
            <div className="flex items-center p-4 space-x-4 bg-white rounded-xl shadow-sm">
                <img src={mockParent.avatarUrl} alt="Parent Avatar" className="w-20 h-20 rounded-full object-cover border-4 border-white shadow-md flex-shrink-0 aspect-square" />
                <div>
