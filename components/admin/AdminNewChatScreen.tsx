...
const Avatar: React.FC<{ contact: Contact }> = ({ contact }) => (
    contact.avatarUrl
        ...
        : <div className="w-11 h-11 rounded-full bg-gradient-to-br from-indigo-200 to-indigo-300 flex items-center justify-center flex-shrink-0 ring-2 ring-white shadow-sm">
            <span className="text-sm font-bold text-indigo-700">{contact.name?.charAt(0)?.toUpperCase() || '?'}</span>
          </div>
);
...
const ContactRow: React.FC<{
    ...
    >
        <div className="relative flex-shrink-0">
            ...
            {selectable && selected && (
                <div className="absolute -bottom-0.5 -right-0.5 w-5 h-5 bg-indigo-500 rounded-full flex items-center justify-center ring-2 ring-white">
        ...
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} transition={{ duration: 0.2 }} className="absolute inset-0 bg-white/95 backdrop-blur-md flex flex-col z-20">
            ...
            <div className="flex-1 p-6 flex flex-col gap-6">
                <div className="flex flex-col items-center gap-3">
                    <div className="w-20 h-20 rounded-full bg-gradient-to-br from-indigo-400 to-indigo-600 flex items-center justify-center shadow-lg">
                ...
                <div>
                    ...
                    <input
                        autoFocus
                        type="text"
                        placeholder="e.g. Staff Notices, Class 10A Parents..."
                        value={name}
                        onChange={e => setName(e.target.value)}
                        maxLength={60}
                        className="w-full px-4 py-3 bg-gray-100/80 border border-gray-200/60 rounded-xl text-sm focus:ring-2 focus:ring-indigo-200 focus:bg-white transition-all outline-none"
            ...
            <div className="p-4 border-t border-gray-100/60">
                <button
                    ...
                    className="w-full py-3.5 bg-indigo-600 hover:bg-indigo-700 disabled:bg-gray-300 text-white font-semibold rounded-xl transition-colors flex items-center justify-center gap-2"
                >
                    {creating
            ...
            <div className="p-4 bg-white/80 backdrop-blur-md border-b border-gray-100/60 sticky top-0 z-10 flex-shrink-0">
                {groupMode ? (
                    <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="font-semibold text-gray-800 text-sm">
                                New Group {selected.length > 0 && <span className="text-indigo-500">· {selected.length} selected</span>}
                            </span>
                        </div>
                        {selected.length > 0 && (
                            <button
                                onClick={() => setShowNameSheet(true)}
                                className="px-4 py-1.5 bg-indigo-600 text-white text-sm font-semibold rounded-full hover:bg-indigo-700 transition-colors"
                            >
                                Next
                            </button>
                        )}
                    ...
                    <div className="flex items-center justify-between mb-3">
                        ...
                        <button
                            ...
                            className="flex items-center gap-1.5 px-3 py-1.5 bg-indigo-50 hover:bg-indigo-100 text-indigo-600 text-xs font-semibold rounded-full border border-indigo-200 transition-colors"
                        >
                ...
                <div className="relative">
                    ...
                    <input
                        ...
                        className="w-full pl-9 pr-4 py-2.5 text-sm bg-gray-100/80 border-none rounded-xl focus:ring-2 focus:ring-indigo-200 focus:bg-white transition-all outline-none placeholder-gray-400 text-gray-700"
                        autoFocus
                    />
                </div>
            </div>
