        ...
        <div className="p-6 max-w-5xl mx-auto space-y-6">
            ...
            <div className="bg-white p-1.5 rounded-2xl shadow-sm border border-gray-100 flex gap-1 w-fit">
                <button onClick={() => setTab('needed')}
                    className={`relative flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-semibold transition-colors ${tab === 'needed' ? 'text-white' : 'text-gray-500 hover:bg-gray-50'}`}>
                    {tab === 'needed' && <motion.div layoutId="substituteTab" className="absolute inset-0 bg-indigo-600 rounded-xl" transition={{ type: 'spring', stiffness: 400, damping: 32 }} />}
                    <span className="relative flex items-center gap-1.5"><UserCheck className="w-4 h-4" /> Coverage Needed {coverage.length > 0 && `(${coverage.length})`}</span>
                </button>
                <button onClick={() => setTab('history')}
                    className={`relative flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-semibold transition-colors ${tab === 'history' ? 'text-white' : 'text-gray-500 hover:bg-gray-50'}`}>
                    {tab === 'history' && <motion.div layoutId="substituteTab" className="absolute inset-0 bg-indigo-600 rounded-xl" transition={{ type: 'spring', stiffness: 400, damping: 32 }} />}
                    <span className="relative flex items-center gap-1.5"><History className="w-4 h-4" /> History</span>
                </button>
            </div>
            ...
            {tab === 'needed' ? (
                coverage.length === 0 ? (
                    <motion.div key="needed-empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-12 text-center">
                    ...
                    <motion.div key="needed" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.2 }} className="space-y-4">
                        {coverage.map((period, pi) => (
                            <motion.div key={period.timetable_id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.2, delay: Math.min(pi, 10) * 0.04 }} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
                                ...
                                {period.suggestions.length === 0 ? (
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-2">
                                        {period.suggestions.map(s => (
                                            <motion.button key={s.teacher_id}
                    ...
                    {history.length === 0 ? (
                        ...
                        <div className="divide-y divide-gray-50">
                            {history.map((h: any, hi: number) => (
                                <motion.div key={h.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.15, delay: Math.min(hi, 15) * 0.02 }} className="px-5 py-4 flex items-center justify-between gap-3">
                                    <div>
