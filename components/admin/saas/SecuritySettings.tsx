...
export const SecuritySettings: React.FC<SecuritySettingsProps> = ({ navigateTo }) => {
    ...
    const checkTwoFactorStatus = async () => {
        try {
            // Read the REAL state from the server. This used to read localStorage,
            // so "2FA enabled" was a flag in the admin's own browser that any other
            // device (or a cleared cache) disagreed with.
    ...
    const generateQRCode = async () => {
        try {
            ...
            // The backend already implements this properly (otplib + qrcode): it
            // generates a per-user secret, stores it unverified, and returns a real
            // QR data URL. This screen used to hand out the hardcoded,
            // publicly-known test secret JBSWY3DPEHPK3PXP instead — so every admin
            // who "enabled" 2FA shared one well-known secret.
    ...
    const verifyAndEnable2FA = async () => {
        ...
        try {
            ...
            // Verified against the stored secret on the server. Previously ANY
            // 6-digit string was accepted and 2FA was "enabled" by writing a
            // localStorage flag — the account gained no second factor at all.
    ...
    const disable2FA = async () => {
        ...
        // anyone with a borrowed session could strip it. Uses prompt() to match
        // the confirm() already above rather than introducing new UI.
                            ...
                            <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.15 }} className="space-y-4">
                                <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
                                    ...
                                    <div className="bg-white p-4 rounded-lg text-center">
                                        {/* Real QR data URL from /auth/2fa/setup. This was a grey
                                            "QR Code Here" placeholder, so Step 1 asked the admin to
                                            scan a code that was never rendered. */}
                                        {qrCode ? (
                                            <img src={qrCode} alt="Two-factor authentication QR code" className="w-48 h-48 mx-auto rounded" />
                                        ) : (
