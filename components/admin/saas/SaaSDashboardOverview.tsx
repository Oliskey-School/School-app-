...
const SaaSDashboardOverview: React.FC<OverviewProps> = ({ navigateTo }) => {
    ...
    if (loading) return <CenteredLoader message="Loading SaaS metrics..." className="p-8" />;
    ...
    return (
        <div className="p-4 sm:p-6 bg-gray-50/50 min-h-full">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-8">
                {cards.map((card, idx) => (
                    <motion.div key={idx} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2, delay: idx * 0.03 }} className="bg-white p-4 sm:p-6 rounded-xl shadow-sm border border-gray-100 flex items-center justify-between">
                        <div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2, delay: 0.1 }} className="bg-white p-4 sm:p-6 rounded-xl shadow-sm border border-gray-200">
                    <h3 className="font-bold text-gray-800 mb-4 text-base sm:text-lg">Quick Actions</h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                        <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => navigateTo('schools', 'Manage Schools')} className="p-3 sm:p-4 bg-indigo-50 hover:bg-indigo-100 rounded-lg text-indigo-700 font-medium text-left transition text-sm sm:text-base">
                            Manage Schools
                        </motion.button>
                        <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => navigateTo('plans', 'Manage Plans')} className="p-3 sm:p-4 bg-purple-50 hover:bg-purple-100 rounded-lg text-purple-700 font-medium text-left transition text-sm sm:text-base">
                            Subscription Plans
                        </motion.button>
                    </div>
                </motion.div>

                <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2, delay: 0.15 }} className="bg-white p-4 sm:p-6 rounded-xl shadow-sm border border-gray-200">
