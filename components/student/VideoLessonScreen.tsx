...
const RelatedResourceCard: React.FC<{ resource: DigitalResource, onClick: () => void }> = ({ resource, onClick }) => {
    ...
    return (
        <motion.button whileHover={{ x: 2 }} whileTap={{ scale: 0.98 }} onClick={onClick} className="w-full flex items-center p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
            ...
            <main className="flex-grow overflow-y-auto">
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 p-4">
                    ...
                    <motion.div
                        ...
                        className="lg:col-span-1 space-y-4 lg:sticky lg:top-6 lg:self-start"
                    >
                        {/* Lesson Notes */}
                        <div className="bg-white rounded-xl shadow-sm overflow-hidden">
                            <button onClick={() => setNotesExpanded(!notesExpanded)} className="w-full flex justify-between items-center p-4 text-left hover:bg-gray-50 transition-colors">
                                <div className="flex items-center space-x-3">
                                    <DocumentTextIcon className="w-6 h-6 text-orange-500" />
                                    <h3 className="text-lg font-bold text-gray-800">Lesson Notes</h3>
                                </div>
                                <ChevronRightIcon className={`h-6 w-6 text-gray-400 transition-transform duration-200 ${notesExpanded ? 'rotate-90' : ''}`} />
                            </button>
                            ...
                            <div className="bg-white p-4 rounded-xl shadow-sm">
                                ...
                                <div className="space-y-2">
                                    {relatedResources.map((res, i) => (
                                        ...
                                        >
                                            <RelatedResourceCard resource={res} onClick={() => handleResourceClick(res)} />
                                        </motion.div>
                                    ))}
                                </div>
                            </div>
